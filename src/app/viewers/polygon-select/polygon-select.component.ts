...
export class PolygonSelectComponent implements AfterViewInit {
    ...
    public setMeshes(meshes: (THREE.Mesh & { targetZ?: number, locked?: boolean, interactive?: boolean, key: string })[]) {
        console.log(`Setting ${meshes.length} meshes`);
        if (this.polygons.size !== 0) {
            console.log("Would set more meshes, returning");
            return;
        }
    ...
    public toggleColorConfigProvider() {
        console.log('Toggling color config provider', this.colorConfigProviders.length);
